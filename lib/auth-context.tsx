...
interface AuthContextValue {
  ...
  client: Client | null;
  assignedClients: { id: string; name: string; cugCode: string }[];
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const [assignedClients, setAssignedClients] = useState<{ id: string; name: string; cugCode: string }[]>([]);
  ...
  useEffect(() => {
    fetch("/api/auth/me")
      ...
      .then((data) => {
        ...
        // Set active client: user's own client, or first assigned, or first available
        const defaultClientId =
          data.user.clientId ||
          data.assignedClients?.[0]?.id ||
          null;
